...
def make_colormip(neuron: Union[int, List[int], str],
    ...
    # Add the standard 90 pixel header (following NeuronBridge & Fiji plugin convention)
    colormip = np.vstack([np.zeros((90, colormip.shape[1], 3), dtype=np.uint8), colormip])
    if not save:
        return colormip
...
def render_neuron_into_template_space(seg_id: Union[int, List[int]],
    ...
    ----------
    seg_id : int
        The segment ID(s) from the FANC segmentation to render.
